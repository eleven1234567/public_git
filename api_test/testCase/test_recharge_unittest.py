...
test_data=MyExcel(path_conf.test_case_path,'recharge','init_data').my_excel()

object=SaveValue()

@ddt
class TestRechargeUnittest(unittest.TestCase):
    def setUp(self):
        self.t=MyExcel(path_conf.test_case_path,'recharge','init_data')
        MyLog().info('--------开始执行测试用例--------')
    ...
    def test_register_unittest(self,item):
        ...
        if item['CheckSql'] != None:
            ...
            recharge_Amount=int(float(after_amount)-float(before_amount))   #充值金额

            try:
                self.assertEqual(object.amount,recharge_Amount)
                MyLog().info('充值成功')
                sql_result = 'PASS'
                ...
                MyLog().info('数据库查询出的充值金额为：{}'.format(int(float(after_amount)-float(before_amount))))
